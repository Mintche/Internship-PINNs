#!/usr/bin/env python3
"""Compare two scattered PINN checkpoints on a fixed FEM P2 node set."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate two scattered checkpoints at the same FEM P2 degrees of "
            "freedom, compare total fields U = u0 + us, and display maps."
        )
    )
...
def _parse_integer(value: str, column: str, filepath: Path) -> int:
    try:
        ...
        raise ValueError(f"Non-numeric value in column {column!r} of {filepath}") from error
    if not np.isfinite(numeric_value) or numeric_value != round(numeric_value):
        raise ValueError(
            f"Invalid integer value in column {column!r} of {filepath}: {value!r}"
        )
...
def validate_geometry(
    checkpoint1: USCheckpoint,
    checkpoint2: USCheckpoint,
    grid: FEMNodeGrid,
) -> None:
...
def get_defect_name(checkpoint1: USCheckpoint, checkpoint2: USCheckpoint) -> str:
...
def select_cases(
    ...
    if not cases:
        raise ValueError("No case shared by the checkpoints matches the requested filters")
...
def compute_misfit_metrics(
    ...
    if values1.ndim != 1 or values1.size != mass_matrix.size:
        raise ValueError(f"Fields must have shape ({mass_matrix.size},), got {values1.shape}")
    ...
    if stiffness_matrix is None:
        return MisfitMetrics(l2_absolute=float(l2_absolute), l2_relative=float(l2_relative))
    ...
    h1_reference_squared = l2_reference_squared + stiffness_matrix.quadratic_form(values1)
...
def prepare_comparisons(
    ...
    for frequency, mode in cases:
        values1 = checkpoint1.predict_total_physical(
            frequency, mode, grid.x, grid.y, physical_units=True
        )
        values2 = checkpoint2.predict_total_physical(
            frequency, mode, grid.x, grid.y, physical_units=True
        )
        metrics = compute_misfit_metrics(values1, values2, mass_matrix, stiffness_matrix)
        results.append(
            ComparisonResult(
                frequency=frequency,
                mode=mode,
...
def main() -> None:
    args = parse_args()
    checkpoint1 = load_us_checkpoint(args.checkpoint1)
    checkpoint2 = load_us_checkpoint(args.checkpoint2)
    ...
    mass_matrix = load_symmetric_coo_matrix(args.mass_matrix, expected_size=grid.size)
