...
# Load boundary data for each mode
# Structure: mode_data[mode_idx] = {
#   'Y_left': array, 'Y_right': array,
#   'U_left_norm': {freq: array}, 'U_right_norm': {freq: array},
#   'U_norm': {freq: scalar}, 'dataset_freq': array
# }
mode_data = {}
...
for mode_idx in training_modes:
    ...
    X_left, Y_left_raw, U_re_left, U_im_left, dataset_freq = loader_left.get_training_data()
    X_right, Y_right_raw, U_re_right, U_im_right, _ = loader_right.get_training_data()

    # Process Y coordinates (normalize to [-1, 1])
    Y_left = Y_left_raw[dataset_freq[0]]
    Y_right = Y_right_raw[dataset_freq[0]]
...
# Wavefield networks: one per (frequency, mode) pair
# params_uv[(freq, mode_idx)] = {'layers': ..., 'sigma': ...}
params_uv = {}
for f in dataset_freq:
    for mode_idx in training_modes:
        key, subkey_f = jax.random.split(key)
        params_uv[(float(f), mode_idx)] = init_layers_uv(subkey_f, n_layers_uv, f, mode_idx)
...
def loss_fn(params_uv, layers_m, x, y, f, mode_index, target_u_left, target_u_right,
            y_bnd_left, y_bnd_right, u_norm_val, weights, is_warmup=False, use_healthy_guide=False):
    def uv(x, y):
...
def make_train_step_inverse_multimode(adam_opt_uv, adam_opt_m, lbfgs_opt_packed):
    ...
    def train_step_inverse(params_uv_list, layers_m, opt_states_uv, opt_state_m,
                           ...
                           current_weights, N, n_modes_used, use_lbfgs=False):
        ...
        # Compute combined loss over all modes
        def loss_all_modes(p_uv_list, l_m):
            total_loss = 0.0
            total_pde = 0.0
            total_bc = 0.0
            total_data = 0.0
            for i in range(n_modes_used):
                l, (pde, bc, data) = loss_fn(
                    p_uv_list[i], l_m, x, y, f_val, mode_indices[i],
                    targets_left[i], targets_right[i],
                    y_bnds_left[i], y_bnds_right[i], u_norms[i],
                    current_weights, is_warmup=False, use_healthy_guide=False)
                total_loss += l
                total_pde += pde
                total_bc += bc
                total_data += data
            # Average over modes
            n = n_modes_used
            return total_loss / n, (total_pde / n, total_bc / n, total_data / n)

        (loss, aux), (grads_uv_list, grads_m) = jax.value_and_grad(
            loss_all_modes, argnums=(0, 1), has_aux=True)(params_uv_list, layers_m)

        if use_lbfgs:
            params_packed = {'uv_list': params_uv_list, 'm': layers_m}
            grads_packed = {'uv_list': grads_uv_list, 'm': grads_m}
            ...
            params_uv_list = params_packed['uv_list']
            layers_m = params_packed['m']
        else:
            # Update each mode's u-network independently
            new_params_uv_list = []
            new_opt_states_uv = []
            for i in range(n_modes_used):
                updates_uv_i, new_opt_state_uv_i = adam_opt_uv.update(
                    grads_uv_list[i], opt_states_uv[i], params_uv_list[i])
                new_params_uv_list.append(optax.apply_updates(params_uv_list[i], updates_uv_i))
                new_opt_states_uv.append(new_opt_state_uv_i)
            params_uv_list = new_params_uv_list
            opt_states_uv = new_opt_states_uv

            # Update shared m-network with accumulated gradients from all modes
            updates_m, opt_state_m = adam_opt_m.update(grads_m, opt_state_m, layers_m)
            layers_m = optax.apply_updates(layers_m, updates_m)

        return params_uv_list, layers_m, opt_states_uv, opt_state_m, opt_state_lbfgs, loss, aux, new_key
...
def train(params_uv, layers_m, N_adam, N_lbfgs,
          ...
          freqs, training_modes, mode_data, eval_interval,
    ...
    for i, freq in enumerate(freqs):
        print(f"\n{'='*60}")
        print(f"--- f = {freq} Hz | modes = {training_modes} ---")
        print(f"{'='*60}")
        ...
        if max_steps_adam_phase2 > 0 or max_steps_lbfgs_phase2 > 0:
            ...
            # Build u-params as a list (required for JAX tracing through the multi-mode loop)
            param_uv_list = [params_uv[(freq, mi)] for mi in training_modes]
            ...
            # Prepare data arrays as lists (one per mode)
            mode_indices = [mi for mi in training_modes]
            targets_left = [mode_data[mi]['U_left_norm'][freq] for mi in training_modes]
            targets_right = [mode_data[mi]['U_right_norm'][freq] for mi in training_modes]
            y_bnds_left = [mode_data[mi]['Y_left'] for mi in training_modes]
            y_bnds_right = [mode_data[mi]['Y_right'] for mi in training_modes]
            ...
            for step in range(max_steps_adam_phase2):
                ...
                param_uv_list, layers_m, opt_states_uv, opt_state_m, opt_state_lbfgs, loss, aux, key = step_inverse(
                    param_uv_list, layers_m, opt_states_uv, opt_state_m, opt_state_lbfgs,
                    key, freq, mode_indices,
                    targets_left, targets_right, y_bnds_left, y_bnds_right, u_norms,
                    weights_phase2, N=N_adam, n_modes_used=n_modes_used, use_lbfgs=False)

                if step % eval_interval == 0:
                    pde_loss, bc_loss, data_loss = aux
                    loss_inverse.append((pde_loss, bc_loss, data_loss))
                    sigmas_str = " | ".join([f"M{mi} σ={param_uv_list[j]['sigma']}" for j, mi in enumerate(training_modes)])
                    ...
                    if loss < best_loss:
                        best_loss = loss
                        best_params_uv_list = [p for p in param_uv_list]
                        best_params_m = layers_m
                    ...
                    if check_switch_criterion(loss_history_p2, window=switch_window, threshold=switch_threshold):
                        ...
                        param_uv_list = best_params_uv_list
            ...
            if switched_to_lbfgs_p2 or max_steps_lbfgs_phase2 > 0:
                ...
                opt_state_lbfgs = lbfgs_packed_p2.init({'uv_list': param_uv_list, 'm': layers_m})
                weights_phase2_lbfgs = jnp.array([1.0, 1.0, 1.0])

                key, lbfgs_key = jax.random.split(key)

                for step in range(max_steps_lbfgs_phase2):
                    param_uv_list, layers_m, opt_states_uv, opt_state_m, opt_state_lbfgs, loss, aux, _ = step_inverse(
                        param_uv_list, layers_m, opt_states_uv, opt_state_m, opt_state_lbfgs,
                        lbfgs_key, freq, mode_indices,
                        targets_left, targets_right, y_bnds_left, y_bnds_right, u_norms,
                        weights_phase2_lbfgs, N=N_lbfgs, n_modes_used=n_modes_used, use_lbfgs=True)
            ...
            # Write back best params to the main dict
            for j, mi in enumerate(training_modes):
                best_params_uv[(freq, mi)] = best_params_uv_list[j]
                params_uv[(freq, mi)] = best_params_uv_list[j]
            layers_m = best_params_m

    return best_params_uv, layers_m, key, loss_forward, loss_inverse
...
params_uv, layers_m, key, loss_forward, loss_inverse = train(
    ...
    max_steps_adam_phase1, max_steps_lbfgs_phase1,
    max_steps_adam_phase2, max_steps_lbfgs_phase2,
    training_frequencies, training_modes, mode_data,
...
plt.colorbar(label="Sound speed c(x, y)")
modes_str = ', '.join([str(mi) for mi in training_modes])
freqs_str = ', '.join([str(int(f)) for f in training_frequencies])
plt.title(f"Reconstructed c(x,y) — Modes [{modes_str}] | Freqs [{freqs_str}] Hz")
plt.tight_layout()
